...
public abstract class AbstractCashRegisterSmartCard implements ICashRegisterSmartCard {
	...
	protected void executeSelectWithFileIdAPDU(byte[] fileID) throws SmartCardException {

		CommandAPDU select = new CommandAPDU(0x00, 0xA4, 0x00, 0x0C, fileID);
		executeCommand(select);
	}

	protected ResponseAPDU selectWithAppliactionId(byte[] appliactionId) throws SmartCardException {
		CommandAPDU select = new CommandAPDU(0x00, 0xA4, 0x04, 0x0C, appliactionId);
		return executeCommand(select);
	}

	protected ResponseAPDU executeCommand(CommandAPDU commandAPDU) throws SmartCardException {

		ResponseAPDU responseAPDU = null;
		try {
			responseAPDU = channel.transmit(commandAPDU);
			if (responseAPDU.getSW() != 0x9000 && responseAPDU.getSW() != 0x6A82) {
				throw new SmartCardException("Response APDU status is " + responseAPDU.getSW());
			}
		} catch (Exception e) {
	...
	protected byte[] getData(CommandAPDU commandAPDU) throws SmartCardException {
	...
	protected List<byte[]> getBuffer(boolean onlyFirst, byte[] DF, byte[] EF) throws SmartCardException, CardException {
